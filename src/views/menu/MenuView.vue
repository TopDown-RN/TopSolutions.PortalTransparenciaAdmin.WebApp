...
<template>
    <div class="p-8">
      <div class="mb-4">
        <input type="text" placeholder="Descrição do menu" class="border rounded-lg p-2" v-model="descricao">
      </div>
  
      <div class="mb-4">
        <input type="text" placeholder="Descrição Geral" class="border rounded-lg p-2" v-model="descricaoGeral">
      </div>
      ...
      <div class="mb-4">
        ...
        <select class="border rounded-lg p-2" v-model="submenu">
          <option v-for="menu in menusSorted" :key="menu.idMenu" :value="menu.idMenu">{{ menu.txtDescricao }}</option>
        </select>
      </div>
          ...
          <tbody>
            <tr v-for="menu in menusSorted" :key="menu.idMenu" class="border border-gray-400">
              ...
              <td class="w-1/4 border border-gray-400 p-2">
                <button class="bg-gray-500 text-white px-2 py-1 rounded-lg" @click="editar(menu)">Editar</button>
              </td>
            </tr>
          </tbody>
        </table>
