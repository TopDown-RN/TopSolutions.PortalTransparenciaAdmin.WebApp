...
<template>
  <div class="max-w-screen-base container overflow-x-auto">
    ...
    <Dialog 
      ...
      style="width: 600px"
    >
    <h2 class="text-lg font-semibold mb-5">Veja como funcionam os principais ambientes e páginas do Ambiente Adminitrativo</h2>
    ...
    <div class="grid gap-4 text-sm">
        ...
        <div class="grid gap-y-4">
            <button
                v-for="item in items"
                :key="item.title"
                class="group flex items-center w-full bg-gray-700 hover:bg-gray-600 rounded-md border border-gray-600 p-3 transition"
                @click="handleRedirect(item.redirect)"
                >
                <img :src="item.image" alt="Image for {{ item.title }}" class="mr-4" style="width: 80px; height: auto;" />
                <div class="flex flex-col">
                    <h3 class="font-semibold text-left">{{ item.title }}</h3>
